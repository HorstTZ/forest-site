...
Para utilizar a variável sítio como independente tentei usar a regressão logística múltipla. Porém, até onde li, ela não considera as autocorrelações entre os dados.
Resolvi testar o modelo Random forest e fazer a predição do sitio utilizando como covariáveis os atributos de terreno


#Random Forest

```{r}
rf_fit <- caret::train(Sitio ~ ELEV + VD + TWI + CNBL + DECLI + PFd + EA + h, data = pontos@data,
...
```{r}
pontos@data$predsitio <- rf_fit$finalModel$predicted
table(pontos$Sitio, predict(ajuste_glm) > 0.5)
```
