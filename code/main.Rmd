...
#Realizar a predição dos sítios via RF
#Comparar predito e observado
...
endCluster()
prediction <- as.factor(calc(x = prediction, fun = nnet::which.is.max)) %>% plot()
```

#Teste: Ajuste entre a altura das árvores e o sítio predito e observado

```{r}
lm(h ~ prediction + Sitio, data = pontos) %>% summary
```
